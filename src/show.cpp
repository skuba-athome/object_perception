...
void convertmsg2img(const sensor_msgs::ImageConstPtr& msg)
{
	...
	{
		cvPutText(img2, objects[i].name.c_str(), objects[i].centroid, &font, cvScalar(0, 0, 255,0));
	}
...
void resetCallback(const std_msgs::String::ConstPtr& msg) {
	if(msg->data == "reset") objects.clear();
}
int main(int argc, char **argv) 
{
	...
	ros::Subscriber sub_reset = n.subscribe("/object/start_search",10,resetCallback);
	ros::spin();
	ros::Rate r(10); // 10 hz
/*
