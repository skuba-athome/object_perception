...
void depthCb( const sensor_msgs::ImageConstPtr& image )
{
	...
	if (start==50){
		...
		depthstart=dist[320][240];		
	}
	//printf("%f    %f  \n",center,depthstart);
	if(((center>depthstart+5)||(center<depthstart-5))&&(depthstart>0)){
		//printf("-------------------------------\n");
		startopen++;
		if(startopen>50){
			//printf("%f    %f  \n",center,depthstart);
			if((center>depthstart+10)||(center<depthstart-10)){
			//printf("door open \n");
			state="open";
			}
		}
...
{
  FILE * imgListFile = 0;
  ros::init(argc,argv,"testb");
  ros::NodeHandle n;
  ros::NodeHandle nh("~");
  nh.param("min_range", min_range_, 0.5);
  nh.param("max_range", max_range_, 5.5);
  ros::Publisher chatter_pub = n.advertise<std_msgs::String>("door_to_voice", 1000);
